...
from utils.networks import AdvantageConditionedActorVectorField, QuantileValue, ActorZ
import wandb
...
class DSRLAgent(flax.struct.PyTreeNode):
    ...
    def _compute_target_quantiles(self, batch):
        ...
        next_v = next_z_dist # B, N
            
        # 3. Bellman Update
        T = batch['rewards'][..., -1][..., None] + \
            (self.config['discount'] ** self.config["horizon_length"]) * \
            batch['masks'][..., -1][..., None] * next_v
    ...
    def compute_z_loss(self, batch, grad_params, rng):
        batch_actions = jnp.reshape(batch["actions"], (batch["actions"].shape[0], -1))  # fold in horizon_length together with action_dim
        batch_size, action_dim = batch_actions.shape
        rng, x_rng, t_rng, drop_rng = jax.random.split(rng, 4)
    ...
    def _compute_distributional_advantage(self, z_q, z_v):
        ...
        z_q = z_q[..., -2:] # Taking implicit max for more improvement.
    ...
    def create(
        ...
        network_info = dict(
            ...
            actor_z=(actor__def, (ex_observations)),
