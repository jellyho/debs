        ...
        for task in "cube-single-play" "cube-double-play" "scene-play" "puzzle-3x3-play" "puzzle-4x4-play"; do
            for alpha in "1.0"; do
                for seed in "100" "200" "300"; do
                    ...
                    sbatch -p ${PART} -q ${QOS} --exclude=node19,node08,node16 --gres=gpu:1 -J ${JOBNAME}_${task}_${seed} -o ~/.slurm_logs/${JOBNAME}_${task}_${seed}.log debs_ogbench.sh ${task} ${MODEL} ${latent} ${alpha} ${JOBNAME} ${task_num} ${seed} ${lahd} ${critic_agg} ${extract_method} ${mf_method}
                done
            done
        done
    done
