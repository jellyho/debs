...
def cross_entropy_loss_on_scalar(logits: jax.Array, target: jax.Array, target_to_dist_fn: jax.Array) -> jax.Array:
    """
    ...
    target: (n_ensemble, batch_size) or (batch_size,)
    """
    if len(target.shape) == 1:
        # Case 1: target shape is (batch_size,)
        target_probs = jax.vmap(
            target_to_dist_fn,
            in_axes=0,
            out_axes=0,
        )(target)  # map over batch dimension
    else:
        # Case 2: target shape is (n_ensemble, batch_size)
        target_probs = jax.vmap(
            jax.vmap(target_to_dist_fn, in_axes=0, out_axes=0),
            in_axes=0,
            out_axes=0,
        )(target)  # map over ensemble and batch dimensions
