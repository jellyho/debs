...
class ImprovedActorMeanFlowField(nn.Module):
    """
    Actor vector field network that conditions on an 'advantage' scalar.
    ...
    def setup(self) -> None:
        # Standard MLP for the vector field
        self.mlp = MLP((*self.hidden_dims[:2], self.action_dim), activate_final=False, layer_norm=self.layer_norm)
        self.mlp_u = MLP((*self.hidden_dims[2:], self.action_dim), activate_final=False, layer_norm=self.layer_norm)
        self.mlp_v = MLP((*self.hidden_dims[2:], self.action_dim), activate_final=False, layer_norm=self.layer_norm)
        # Fourier Features for Timestep (t)
        if self.use_fourier_features:
            self.ff = FourierFeatures(self.fourier_feature_dim)
