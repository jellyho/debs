...
export JOBNAME="CFGRL_VISUAL_FINAL"
export MODEL="cfgrl"
    ...
    for latent in "normal"; do
        for task in "cube-single-play"; do
            for seed in "100" "200" "300"; do
                for alpha in "1.25"; do
                    echo "${JOBNAME}_${task_num}_${latent}_${task}_${seed}"
                    sbatch --exclude=node19,node16 -p ${PART} -q ${QOS} --gres=gpu:1 -J ${JOBNAME}_${task}_${seed} -o ~/.slurm_logs/${JOBNAME}_${task}_${seed}.log cfgrl_visual_ogbench.sh ${task} ${MODEL} ${latent} ${alpha} ${JOBNAME} ${task_num} ${seed}
                done
            done
        done
    done
