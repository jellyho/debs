...
export JOBNAME="BANDIT_test_target_real"
# export MODEL="meanflow"


for model in "meanflowrf"; do
    ...
    for latent in "normal" "sphere"; do
        for num in "1" "2"; do
            for seed in "100"; do
                ...
                sbatch -p ${PART} -q ${QOS} --gres=gpu:1 -J ${JOBNAME}_${seed} -o ~/.slurm_logs/${JOBNAME}_${seed}.log bandit_debs.sh ${num} ${model} ${latent} 1.0 ${JOBNAME}
            done
        done
    done
done
